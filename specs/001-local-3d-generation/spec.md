...
**Input**: User description: "Authorized users submit an image to a private local 3D-generation service, follow its progress, preview the textured GLB, and download the result through a protected Internet boundary."

## Clarifications

### Session 2026-09-02

- Q: Should the IP values in the retained authentication-exemption memo be treated as this project's current Public IP? → A: Yes. The owner designates those retained IP values as the current Public IP baseline for this project; verify reachability and routing only at the Public Deployment gate.
- Q: How will the MVP authorize public users and isolate job access? → A: Shared credentials at the public entry point plus one cryptographically random access token per job; no application user accounts in MVP.
...
An authorized user uploads one valid reference image, starts a generation job, receives a unique job reference, follows it until it finishes, previews the resulting textured 3D asset, adjusts the view with rotate, zoom, pan, and reset controls, then downloads the same asset.

**Why this priority**: This is the smallest complete user outcome: transform an image into a usable downloadable 3D asset.

**Independent Test**: Using one valid supported image, an authorized evaluator can submit a job, identify it by its job reference, reach a completed state, interact with its preview, and download one valid 3D asset without operator help.

**Acceptance Scenarios**:

1. **Given** an authorized user and a valid supported image, **When** the user submits it for generation, **Then** the service accepts it and returns one opaque job reference.
...
An authorized user sees whether a submitted job is queued, processing, completed, failed, or cancelled. The user can refresh or reconnect while the job is active and receives a safe, understandable result when processing cannot finish.
...
Multiple authorized users can submit jobs while the single processing resource is busy. Each job waits or processes safely, with isolated input, output, status, and download access.
...
**Independent Test**: Two authorized evaluators submit distinct images while one job is active. Each evaluator can see and download only their own result, and no file or status data crosses between jobs.
...
An authorized user can perform the core flow through HTTPS from an external network, while internal application, processing, storage, and administration interfaces remain unavailable from the Internet.

**Why this priority**: The product is valuable only when authorized external users can use it without exposing the local server's internal services.
...
1. **Given** an authorized external user, **When** the user opens the service via HTTPS, **Then** the user can complete the P1 flow through the public entry point.
...
3. **Given** a user who lacks the approved minimum access control, **When** that user attempts to use the service, **Then** the service refuses access before a generation job is accepted.
...
- **FR-001**: The service MUST permit an authorized user to submit one supported reference image for 3D asset generation.
...
- **FR-005**: The service MUST let an authorized user retrieve the current state and all available supported progress or queue information for their job after a browser refresh or reconnect.
...
- **FR-013**: The service MUST retain enough job history for authorized users to recover job lookup after refresh and for the operator to recover non-terminal work after a restart.
...
- **FR-016**: Public access MUST require shared credentials at the public entry point. Every accepted job MUST additionally receive a cryptographically random access token, and that token MUST be required for the job's status, preview, generated asset, and download. University/network authentication exemption is separate from application access control. Application user accounts, registration, password recovery, and role management MUST remain out of scope for MVP.
...
- **Access Policy**: The owner-approved rule that determines who may submit, inspect, preview, or download a job.
...
- **SC-001**: In acceptance testing, 100% of valid authorized test submissions complete the user flow from upload to preview and download or reach a safe, explainable terminal failure state; no job remains indefinitely non-terminal.
...
- **SC-004**: An authorized evaluator can refresh or reconnect during queued and processing states and recover the same job's current state in every documented acceptance scenario.
- **SC-005**: In external-network acceptance testing, authorized users can use the complete flow through HTTPS while probes to every documented internal port fail.
...
- **SC-007**: The operator can trace every acceptance-test job from submission to terminal outcome using its job reference without reading the uploaded image or exposing credentials.
...
- The owner approved shared public-entry credentials plus per-job tokens, local
  SQLite persistence, and the JPEG/PNG 10 MiB, 24-hour retention, 10%-free-disk
  admission policy for MVP.
- By owner decision, the retained authentication-exemption memo supplies the
  current Public IP baseline for this project. That decision does not transfer
  the memo's Authentication exemption to this project.
