...
- [X] T059 Install and pin ComfyUI plus required custom-node revisions in `workflows/hunyuan3d/workflow-manifest.json`; complete only when recorded commits/hashes match the running `127.0.0.1:8188` instance and a post-restart health request succeeds—configuration text alone is insufficient. **Requires Windows NVIDIA server evidence.**
- [X] T060 Execute PyTorch/CUDA/native-wheel compatibility checks from `scripts/windows/verify_hunyuan_runtime.ps1` and store output in `evidence/windows/runtime-compatibility.md`; complete only when CUDA is available, the intended GPU is selected, imports succeed, and no dependency is silently upgraded. **Requires Windows NVIDIA server evidence.**
- [X] T061 Export and run the native Hunyuan3D 2.1 shape smoke workflow in `workflows/hunyuan3d/editable/hunyuan3d-21-shape-smoke.json` and `workflows/hunyuan3d/api/hunyuan3d-21-shape-smoke.json`; complete only when API submission generates a parseable shape artifact recorded in `evidence/windows/shape-smoke.md`, explicitly marked not MVP textured-GLB completion. **Requires Windows NVIDIA server evidence.**
- [X] T062 Implement and execute manifest/hash/`/object_info` compatibility verification in `scripts/verify/verify_comfy_manifest.py`; complete only when the pinned running instance passes and deliberate node/hash mismatch fixtures fail closed with evidence in `evidence/windows/object-info-check.md`. **Requires Windows NVIDIA server evidence.**
- [X] T063 Create and execute the Windows GPU generation checklist in `docs/operations/windows-gpu-validation.md`; complete only when every prerequisite and shape-smoke item has command output/artifact paths and an explicit PASS/FAIL/BLOCKED verdict. **Requires Windows NVIDIA server evidence.**
- [X] T064 Issue the compatibility gate verdict in `evidence/windows/phase-7-gate.md`; complete only when T058–T063 are PASS, or record BLOCKED with the exact failing dependency and smallest owner action. **Requires Windows NVIDIA server evidence.**
...
- [X] T068 [US1] Create the hardware-gated real adapter smoke test in `apps/api/tests/integration/test_comfy_adapter_smoke.py`; complete when test collection passes on macOS with a documented skip and fails at the expected missing adapter/runtime assertion on the Windows test environment. **Requires Windows NVIDIA server evidence.**
...
- [X] T076 [US1] Validate the real result with `scripts/verify/validate_glb.py` and record mesh, primitive, UV, material, texture, size, and SHA-256 data in `evidence/windows/textured-glb-validation.md`; complete only when every required GLB property passes. **Requires Windows NVIDIA server evidence.**
- [X] T077 [US3] Execute two distinct jobs serially and capture adapter concurrency, input/output paths, Job IDs, hashes, duration, peak VRAM, and overwrite checks in `evidence/windows/two-job-serial.md`; complete only when both textured GLBs pass validation, remain isolated, and maximum active GPU jobs equals one. **Requires Windows NVIDIA server evidence.**
- [X] T078 [US2] Execute controlled engine failure, timeout, disconnect, missing output, backend restart, and ComfyUI restart cases using `scripts/windows/run_recovery_matrix.ps1`; complete only when `evidence/windows/recovery-matrix.md` shows safe terminal/reconciled states and zero duplicate executions. **Requires Windows NVIDIA server evidence.**
- [X] T079 [US1] Record the real-GPU phase verdict and pinned revision/hash set in `evidence/windows/phase-9-gate.md`; complete only when T075–T078 are PASS and shape-only evidence is not used as textured completion. **Requires Windows NVIDIA server evidence.**
...
- [X] T084 [US1] Record the LAN gate verdict in `evidence/lan/phase-10-gate.md`; complete only when T080–T083 are PASS with commands, timestamps, Job IDs, logs, screenshots, and GLB hashes. **Requires Windows NVIDIA server evidence.**
...
- [x] T086 [P] [US4] Write Caddy configuration contract tests for HTTPS-only public entry without `basic_auth`, request-body limit, upstream restricted to the WireGuard tunnel address, job-token log redaction, a maintenance-page fallback, and the `161.200.90.3` exclusion in `tests/security/test_caddy_contract.py`; complete when `python -m pytest tests/security/test_caddy_contract.py` passes against the authored (not yet deployed) Caddy configuration. **Requires owner approval or owner-provided access.** — Written and passing against `deploy/caddy/Caddyfile` (2026-09-04); Caddy has NOT been deployed or exposed.

### Deployment and verification

- [ ] T087 [US4] Implement the Caddy configuration for the approved hostname without `basic_auth`, upstream restricted to the WireGuard tunnel address (`10.10.0.2:3000`), and a `handle_errors` maintenance-page fallback in `deploy/caddy/Caddyfile` and `deploy/caddy/.env.example`; complete only when `caddy validate --config deploy/caddy/Caddyfile` (run on the edge server), T086, public HTTPS routing, and job-token forwarding tests pass without committing secrets. **Requires owner approval or owner-provided access.** — Authored 2026-09-04 (`deploy/caddy/Caddyfile`, `deploy/caddy/maintenance/maintenance.html`); `caddy validate` not yet run (Caddy not installed on the edge server; edge machine/OS not yet confirmed — see owner-gate).
- [ ] T088 [US4] Implement least-privilege boundary rules on both machines — `deploy/firewall/configure-upstream-boundary.ps1` (laptop: opens 3000/tcp to the edge's tunnel address only, refuses if the laptop holds a public IP) and `deploy/firewall/configure-public-edge.ps1` (edge: opens 443/tcp+80/tcp to Caddy only, 51820/udp for WireGuard, and a management port to a trusted source only; hard-refuses any `-PublicAddress` other than `161.200.90.4`) plus read-only verifiers `deploy/firewall/verify-upstream-boundary.ps1` and an edge-side equivalent; complete only when the verifiers record 443/80/51820 correctly scoped, 3000/8000/8188/3389/2019 blocked, and evidence written to `evidence/public-deployment/firewall.md` (and `firewall-upstream.md`). **Requires Windows NVIDIA server evidence. Requires owner approval or owner-provided access.** — Scripts authored 2026-09-04; not yet run against a live deployment (no server holds `161.200.90.4` under operator control yet; management path per Stage 0.1 of the deployment plan not yet decided).
- [ ] T089 [US4] Apply owner-approved DNS and confirm the university border firewall permits inbound 443/tcp, 80/tcp, and 51820/udp to `161.200.90.4` (there is no home/office router in this topology — the address is assigned directly to a university-network edge server, so "router forwarding" is replaced by border-firewall permission); record redacted before/after evidence in `evidence/public-deployment/dns-router.md`; complete only when public DNS resolves to the revalidated current Public IP, no CGNAT/routing blocker remains, and no internal port forward exists (the WireGuard tunnel to the GPU laptop is not a port forward). **Requires owner approval or owner-provided access.**
- [ ] T090 [US4] Obtain and validate the public certificate and redirect behavior with `scripts/verify/test_https_boundary.py --confirm-off-campus`; complete only when `evidence/public-deployment/tls.md` records trusted hostname validation, HTTPS 443 success, HTTP 80 redirect/certificate-only behavior, no certificate warnings, and the bare public IP does not serve the application. **Requires owner approval or owner-provided access.** — Script authored 2026-09-04; refuses to record a PASS without `--confirm-off-campus` and refuses any `--public-ip` other than `161.200.90.4`.
- [ ] T091 [US4] Execute public-entry and per-job-token boundary tests from an external client using `scripts/verify/test_public_auth.py --confirm-off-campus`; complete only when HTTPS access and valid submission work without a site-wide login, missing/wrong-job tokens return uniform 404 for job resources, and no token appears in captured URLs or the Caddy access log in `evidence/public-deployment/auth.md`. **Requires owner approval or owner-provided access.** — Script authored 2026-09-04.
- [ ] T092 [US4] Execute an external TCP/UDP service scan using `scripts/verify/test_external_ports.py --confirm-off-campus` and `scripts/verify/test_wireguard_reachability.py` (both `--mode negative`, from off-campus, and `--mode positive`, run on the GPU laptop — a UDP scan alone cannot distinguish "WireGuard working" from "firewall closed," so both modes are required); complete only when `evidence/public-deployment/ports.md` and `evidence/public-deployment/wireguard.md` show expected 443/80/51820 behavior and failed connections to 3000, 8000, 8188, 3389, and 2019, with `161.200.90.3` never probed. **Requires owner approval or owner-provided access.** — Scripts authored 2026-09-04.
...
- [ ] T093 [US4] Execute the external-network full-flow acceptance checklist in `docs/operations/external-acceptance.md` using `scripts/verify/test_external_acceptance.py --confirm-off-campus`; complete only when `evidence/public-deployment/full-flow.md` records a public submission, real queue/process state, textured preview with rotate/zoom/pan/reset, and a GLB download through HTTPS using the returned job token whose sha256 matches the source file generated on the GPU laptop. **Requires Windows NVIDIA server evidence. Requires owner approval or owner-provided access.** — Script authored 2026-09-04.
- [ ] T094 [US4] Create and execute the external-network security checklist in `docs/operations/external-network-security-checklist.md`, plus the project's core mobility acceptance test using `scripts/verify/test_mobility.py --confirm-off-campus` (the GPU laptop must serve real generations after moving from the university to a mobile hotspot to a home network, after an off-campus reboot, and must show a maintenance page — not a raw 502 — when disconnected, with **zero** changes to DNS, the public IP, the Caddyfile, or WireGuard addressing throughout); complete only when `evidence/public-deployment/negative-cases.md` and `evidence/public-deployment/mobility.md` record safe expected responses, zero information leakage, and every mobility scenario passing. **Requires owner approval or owner-provided access.** — `test_mobility.py` authored 2026-09-04; this is the project's stated core success criterion, not an optional extra.
