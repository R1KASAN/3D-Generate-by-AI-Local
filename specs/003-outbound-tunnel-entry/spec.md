...
**Status**: Implementation in progress — local implementation partially complete; production deployment and live validation blocked pending Approved Origin access and external approvals.
...
### Session 2026-09-06 — compute-link transport clarification

- Q: Does “no inbound” prohibit the private-link transport listener, or only Internet-facing application and management listeners? → A: **Option A.** “No inbound” means no Internet-facing application or management listener. A narrowly scoped WireGuard UDP transport listener is permitted on the approved origin because the laptop must initiate the link from changing networks while the origin remains the responder. The listener MUST be restricted to the approved origin address and WireGuard UDP port, admitted only by the registered WireGuard peer key, and isolated from Feature 001, ComfyUI, administration, storage, and every other origin service. No new public application or management port, router forwarding, or alternate public route is allowed. This clarification preserves the laptop's mobility, keeps public application traffic on the Approved Origin path, and resolves the C1/O1 wording conflict.
...
**Operator remediation review, 2026-09-06 — Option A consistency pass** (specification, planning, and task artifacts only; no application code, firewall state, WireGuard runtime configuration, provider configuration, or live infrastructure was changed):

- Inbound wording that would have prohibited the approved WireGuard transport listener was normalized in FR-033, SC-008a, SC-013, User Story 2's independent test and acceptance scenario 5, and the corresponding tasks and runbooks. The application and management boundary is unchanged and is not weakened by this wording change.
- The WireGuard firewall and configuration posture is no longer treated as "unchanged from feature 002." A feature-003 origin transport boundary is now specified (SC-018) with its own test-first configuration, static verifier, negative test, and blocked live-origin evidence task.
- Laptop mobility was promoted out of clarification prose into FR-040 and SC-017, with its own manual mobility task.
- Direct WireGuard UDP reachability of the approved origin became an explicit Option A feasibility and cutover precondition (FR-041) rather than an unstated assumption.
...
**Constitution alignment note**: Constitution Principle III (v1.2.0) states that "HTTPS on port 443 is the sole Internet-facing application entry point." This feature has **no Internet-facing application or management listener**. The only permitted inbound socket is the private-link WireGuard transport listener defined in the compute-link contract; it carries authenticated peer traffic and cannot serve Feature 001, ComfyUI, database, storage, metrics, or administration. The outbound connector still owns the public application route, and no application or management port is opened. This is consistent with the principle's private-service boundary and preserves the laptop's changing-network mobility.
...
**Independent Test**: Reboot each machine three consecutive times — the approved origin and the GPU laptop, independently and in both orders — then interrupt and restore the origin's Internet connection. In each trial the layered health check recovers automatically or names the specific failing layer, and no direct Internet-facing application or management listener is permitted at any point. The narrowly scoped WireGuard UDP transport listener defined by the Option A compute-link contract (C1a) is explicitly excluded from this prohibition.
...
5. **Given** the approved origin is entirely offline or the connector cannot carry traffic, **When** a visitor opens the site, **Then** the provider's own error response is returned, no direct Internet-facing application or management listener is exposed — The narrowly scoped WireGuard UDP transport listener defined by the Option A compute-link contract (C1a) is explicitly excluded from this prohibition. — and the operator runbook identifies that response as an origin-down signal.
...
- **FR-003**: The approved origin MUST initiate the provider connection outbound. Production deployment MUST NOT require router port forwarding or a new inbound application rule on the origin. The private compute link MAY use its narrowly scoped WireGuard UDP transport listener on the approved origin; that transport listener is not an application or management entry point.
- **FR-004**: Direct Internet connections to the origin's web entry, backend, AI engine, database, storage, metrics, or administration interfaces MUST return no project application response. The only permitted Internet-reachable socket on the origin is the authenticated WireGuard transport listener defined by the compute-link contract; it MUST not forward or expose any application or management service.
...
- **FR-040**: The GPU laptop MUST be able to move between distinct external networks — university Wi-Fi, a home network, a mobile hotspot, or any other network that permits outbound UDP — and re-establish the private compute link automatically, with no edit to the public DNS record, the public hostname, the provider tunnel route, the approved origin's configuration, the feature 001 application configuration, or the WireGuard tunnel addressing. Mobility is a design obligation of the compute-link contract, not an operational convenience: the origin peer configuration MUST carry no `Endpoint` line and the laptop MUST remain the outbound initiator, so the laptop's current address is learned from each handshake rather than configured. Editing any of those six items in order to restore connectivity after a move is a failure of this requirement, not an accepted workaround for it.
- **FR-041**: Option A's raw WireGuard responder arrangement is valid **only if** the approved origin can receive the selected WireGuard UDP transport directly on its approved network path, without router port forwarding. Confirming this is a feasibility precondition of the compute-link contract and a cutover precondition under FR-033. If target inspection shows the approved origin cannot directly receive that transport without prohibited router forwarding, Option A MUST be declared not feasible for that target: production MUST remain blocked and the compute link MUST be redesigned on a separate outbound-initiated or NAT-traversing transport. The project MUST NOT silently fall back to router port forwarding, a public application port, a public listener on the GPU laptop, or a paid relay or VPS. Until inspection resolves it, this reachability status is `PENDING TARGET INSPECTION` and MUST NOT be recorded as satisfied.
...
- **SC-008a**: With the approved origin intentionally powered down, the public hostname returns the provider's error response, no direct Internet-facing application or management listener on either machine becomes reachable — The narrowly scoped WireGuard UDP transport listener defined by the Option A compute-link contract (C1a) is explicitly excluded from this prohibition. — and service resumes automatically when the origin returns.
...
- **SC-013**: Revoking the active connector credential stops that connector from carrying traffic, and installing its replacement restores the same public hostname without exposing any direct Internet-facing application or management listener. The narrowly scoped WireGuard UDP transport listener defined by the Option A compute-link contract (C1a) is explicitly excluded from this prohibition.
...
- **SC-017**: The GPU laptop reconnects and completes the public journey after moving between at least two distinct external networks — for example university or home Wi-Fi and a mobile hotspot — while the public DNS record, public hostname, provider tunnel route, approved-origin configuration, feature 001 application configuration, and WireGuard tunnel addressing are all verified unchanged before and after each move.
- **SC-018**: The approved origin's WireGuard transport boundary is verified to permit exactly one WireGuard UDP port on the approved origin address, carrying the WireGuard transport/service only, admitted by registered peer public key and scoped to `10.10.0.2/32` on the origin and `10.10.0.1/32` on the laptop, with no TCP application listener, no SSH/RDP/VNC or other management listener, no catch-all inbound rule, no router port forwarding, and no direct GPU-laptop Internet exposure. A deliberately introduced unrelated inbound listener, widened peer scope, or catch-all rule causes that verification to fail rather than pass silently.
...
- **`mangosgo.com` authorization and Cloudflare zone access — single authoritative dependency.** Production naming requires all of: (a) written authorization from the `mangosgo.com` owner confirming that this non-commercial 3D-generation project may use `twin3dgen.mangosgo.com`; (b) the Cloudflare account that already manages the `mangosgo.com` zone, on a plan that supports the required published-application route at zero charge; and (c) either delegated access sufficient for the project operator to create the tunnel DNS route, or the domain owner performing that route creation. The project does not need, and MUST NOT request, the owner's account credentials or passwords. Turnaround is outside project control; implementation continues in parallel under FR-012a and only cutover is gated (FR-011, FR-033, FR-036). This single bullet supersedes the three separate authorization, account, and route bullets that previously restated it.
- A public hostname whose DNS zone is already fully hosted in the Cloudflare account that will create the tunnel route. Cloudflare's partial (CNAME) setup, which would keep DNS at an external provider, is a Business-plan feature and is excluded by the cost gate; the tunnel's `<UUID>.cfargotunnel.com` target proxies only for DNS records inside the same Cloudflare account. `mangosgo.com` satisfies this technical condition because its zone is already managed in Cloudflare by its owner, so no delegation or registration step is required — only the authorization and route creation named in the dependency above.
...
- Direct WireGuard UDP reachability of the approved origin on its approved network path, without router port forwarding. Status: `PENDING TARGET INSPECTION`. If inspection shows it is unavailable, Option A is not feasible for this target and the compute-link transport must be redesigned rather than worked around (FR-041).
- At least two distinct external networks the GPU laptop can physically join — for example university or home Wi-Fi and a mobile hotspot — plus a live public route, in order to evidence FR-040 and SC-017.
