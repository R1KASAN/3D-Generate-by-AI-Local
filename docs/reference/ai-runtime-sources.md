...
| [NVIDIA CUDA Toolkit 12.6 archive for Windows](https://developer.nvidia.com/cuda-12-6-0-download-archive?target_os=Windows&target_arch=x86_64&target_version=11&target_type=exe_local) | Checking a CUDA 12.6 prerequisite for a specific wrapper/native wheel | Installing the toolkit does not by itself establish PyTorch/driver compatibility; verify with `nvidia-smi` and the actual Python environment. |
