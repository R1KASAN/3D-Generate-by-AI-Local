...
**WireGuard transport boundary, 2026-09-06 (T056a/T056b/T058a/T058b/T058c).** The declared boundary in `deploy/wireguard/origin-transport-boundary.md` is now validated against every C1a-1 rule by `scripts/verify/test_wireguard_transport_boundary.py`, with `tests/security/test_wireguard_transport_boundary_contract.py` pinning the checked-in declaration and 12 negative fixtures (widened peer scope, an added application/management listener, a catch-all rule, a router forward, each individually rejected). `pytest tests/security -q` → 113 passed, 1 skipped. The CLI verifier exits 0 and writes `evidence/public-deployment/wireguard-transport-boundary.md`. This proves the *declared* boundary is internally consistent and that the verifier fails closed; it is not a substitute for T066a's origin-local evidence that the running firewall matches the declaration, nor for T066b's finding on direct UDP reachability (FR-041).
